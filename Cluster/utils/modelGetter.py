...
def model_getter(args: argparse.Namespace):
    ...
    if args.model_parameters == 'duong':
        # * Model parameters of "2025 - Duong et al - Telegraphers"
        return UNetModel(
    ...
    elif args.model_parameters == 'dhariwal':
        # * Model parameters of "2021 - Nichol Dhariwal - Improved DDPMs"
        return UNetModel(
            image_size=data.data_dims.size,
            in_channels=data.data_dims.channels, out_channels=data.data_dims.channels,
            model_channels=128, num_res_blocks=3,
            dropout=0.1, attention_resolutions=(16,8),
            num_heads=4, num_head_channels=64, channel_mult=(1,2,2,2)
        )

    elif args.model_parameters == 'ho':
        # * Model parameters of "2020 - Ho et al - DDPM"
        # ! did not specify amount of heads or amount of head channels, or base widths
        return UNetModel(
            image_size=data.data_dims.size,
            in_channels=data.data_dims.channels, out_channels=data.data_dims.channels,
            model_channels=128, num_res_blocks=2,
